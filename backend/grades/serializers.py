...
class StudentSerializer(serializers.ModelSerializer):
    ...
    def get_grades(self, obj):
        ...
        enrollments = obj.enrollments.filter(course=course).prefetch_related(
            "grades__assignment"
        )
        grades = []
...
class AssignmentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if not course_id:
            raise serializers.ValidationError(
                {
                    "course_id": "This field is required when creating an assignment directly."
                }
            )
...
class CourseSerializer(serializers.ModelSerializer):
    ...
    def get_students(self, obj):
        ...
        if "students" in include:
            enrollments = obj.enrollments.prefetch_related(
                "student__user", "grades__assignment"
            ).all()
    ...
    def create(self, validated_data):
        ...
        """
        assignments_data = validated_data.pop(
            "writable_assignments", []
        )  # Get assignments from request
...
class EnrollmentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not course:
            raise serializers.ValidationError(
                {"course_id": "The specified course does not exist."}
            )
        ...
        if self.instance:
            ...
            if (
                Enrollment.objects.filter(student=student, course=course)
    ...
    def create(self, validated_data):
        ...
        return Enrollment.objects.create(
            student=student, course=course, **validated_data
        )
...
class GradeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if enrollment_id:
            ...
            if not enrollment:
                raise serializers.ValidationError(
                    {"enrollment_id": "The specified enrollment does not exist."}
                )
        ...
        if assignment_id:
            ...
            if not assignment:
                raise serializers.ValidationError(
                    {"assignment_id": "The specified assignment does not exist."}
                )
        ...
        if enrollment_id and assignment_id:
            ...
            if assignment.course != enrollment.course:
                raise serializers.ValidationError(
                    "The assignment does not belong to the course in the enrollment."
                )
    ...
    def create(self, validated_data):
        ...
        return Grade.objects.create(
            enrollment=enrollment, assignment=assignment, **validated_data
        )
